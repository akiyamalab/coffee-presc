...
namespace logs{
  ...
  // verbosity control: 0=errors only, 1=info+ (default), 2=debug+
  void set_verbosity(int v);
  // `verbosity`: 0=errors only, 1=info+ (default), 2=debug+
  void log_init(const std::string& filename, int verbosity = 1);
  void close();
