...
namespace format{
  ...
  fragdock::Fragment toFragment(const OpenBabel::OBMol &c_obmol, const uint fragid){
    OpenBabel::OBMol &obmol = const_cast<OpenBabel::OBMol&>(c_obmol);
    std::vector<fragdock::Atom> fatoms(obmol.NumAtoms());
    ...
    // fragdock::Fragment frag(fatoms, obmol.GetTitle(), OpenBabel::canonicalSmiles(obmol));
    fragdock::Fragment frag(fragid, fatoms, obmol.GetTitle());
    for(int i = 0; i < obmol.NumBonds(); i++){
      frag.append(fragdock::Bond(obmol.GetBond(i)->GetBeginAtom()->GetId(),
				obmol.GetBond(i)->GetEndAtom()->GetId(),
...
namespace OpenBabel{
  ...
  */
  ...
  std::string getProperty(OpenBabel::OBMol& mol, const std::string& key){
    if (mol.HasData(key)) {
      return mol.GetData(key)->GetValue();
    }
    else {
      assert(0);
    }
